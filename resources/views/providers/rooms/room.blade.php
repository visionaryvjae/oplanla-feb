...
@include('providers.rooms.components.action-buttons')

<div class="md:max-w-7xl w-full border-red-300 md:px-0 px-2 mx-auto py-10 sm:px-6 lg:px-8">
    @if(session('error'))
    ...
    <div class="mb-6">
        <a href="{{ route('provider.rooms.index') }}" class="text-sm font-medium hover:text-purple-800 flex items-center">
            <svg class="w-4 h-4 mr-1" fill="none" stroke="currentColor" viewBox="0 0 24 24"><path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M15 19l-7-7 7-7"></path></svg>
            Back to Rooms
        </a>
    </div>

    <div class="bg-white px-4 rounded-xl shadow-lg overflow-hidden">
        <div class="md:px-6 md:py-5 px-2 py-4 border-b border-gray-200">
            <div class="flex flex-wrap items-center md:justify-between -mt-2 -ml-4">
                <div class="md:mt-2 md:ml-4">
                    ...
                    <div class="mt-1 flex items-center">
                        ...
                        <p class="text-sm text-gray-600 truncate">{{ $room->provider->booking_address }}</p>
                    </div>
                </div>
        ...
        @php
            $displayImage = $room->provider->photos->where('area', 'display')->first()->image ?? null;
        @endphp
        <div class="grid grid-cols-1 lg:grid-cols-5 gap-8 p-6">
            <div class="lg:col-span-3">
                <div class="aspect-w-16 aspect-h-9 rounded-lg overflow-hidden bg-gray-100">
                    <img class="object-cover w-full h-full" src="{{ $displayImage ? asset('storage/images/' . $displayImage) : asset('storage/images/no-image.png') }}" alt="Main image of the room">
                </div>
            </div>
        ...
        @php
            $roomfacilities = $room->room_facilities ? explode(',', $room->room_facilities) : []; 
            $providerfacilities = $room->provider->provider_facilities ? explode(',', $room->provider->provider_facilities) : []; 
