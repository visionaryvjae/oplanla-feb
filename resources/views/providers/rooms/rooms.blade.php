...
<div class="max-w-7xl mx-auto py-10 sm:px-6 lg:px-8">
    <div class="bg-white rounded-xl shadow-lg overflow-hidden mb-6">
        <form action="{{ route('provider.rooms.index') }}" method="GET" class="px-6 py-4 bg-gray-50 border-b border-gray-200">
            <div class="grid grid-cols-1 md:grid-cols-4 gap-4">
                ...
                <div>
                    ...
                    <select name="available" class="mt-1 block w-full p-2 border-gray-300 rounded-md shadow-sm focus:ring-indigo-500 focus:border-indigo-500 sm:text-sm">
                        ...
                        <option value="not available" {{ request('available') == 'not available' ? 'selected' : '' }}>Not available</option>
                    </select>
                </div>
                <div>
                ...
                <div>
                    <label class="block text-xs font-medium text-gray-700 uppercase">Room Type</label>
                    <select name="rental" class="mt-1 block w-full p-2 border-gray-300 rounded-md shadow-sm focus:ring-indigo-500 focus:border-indigo-500 sm:text-sm">
                        <option value="">Select room type</option>
                        <option value="1" {{ request('rental') ==  1 ? 'selected' : '' }}>Rental</option>
                        <option value="0" {{ request('rental') == 0 ? 'selected' : '' }}>Short Term Accommodation</option>
                    </select>
                </div>
                <div class="flex items-end space-x-2">
                    ...
                    <a href="{{ route('provider.rooms.index') }}" class="inline-flex items-center px-4 py-2 bg-gray-200 border border-transparent rounded-md font-semibold text-xs text-gray-700 uppercase tracking-widest hover:bg-gray-300 transition ease-in-out duration-150">
                        Reset
                    </a>
                </div>
            </div>
        ...
        <div class="px-6 py-5 border-b border-gray-200">
            <div class="flex flex-wrap items-center justify-between -mt-2 -ml-4">
                ...
                <div class="mt-4 ml-4 flex-shrink-0">
                    ...
                    <a href="{{ route('provider.rooms.create.single', ['id' => $pid]) }}" class="inline-flex items-center px-4 py-2 border border-transparent rounded-md shadow-sm text-sm font-medium text-white bg-indigo-600 hover:bg-indigo-700 focus:outline-none focus:ring-2 focus:ring-offset-2 focus:ring-indigo-500">
                        Add New Room
                    </a>
                </div>
            </div>
                                ...
                                @forelse ($rooms as $room)
                                    <tr class="hover:bg-gray-50 cursor-pointer" data-room-id="{{ $room->id }}">
                                        ...
                                        <td class="px-6 py-4 whitespace-nowrap">
                                            <div class="text-sm font-medium text-gray-900">{{ $room->property->name ?? $room->provider->provider_name }}</div>
                            ...
                            @forelse ($rooms as $room)
                                <div class="bg-white p-4 rounded-lg border shadow-sm space-y-3 cursor-pointer" data-room-id="{{ $room->id }}">
                                    <div class="items-center flex justify-between mb-2">
                                        <div>
                                            <p class="text-sm font-semibold text-gray-800">{{ $room->property->name ?? $room->provider->provider_name }}</p>
...
@include('providers.rooms.components.room-select')
