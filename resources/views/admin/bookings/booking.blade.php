{{-- resources/views/Booking/booking.blade.php --}}
...
<script>
    function getRoom_bookingIdFromUrl(buttonClass, routeTemplate) {
        ...
        const room_bookingidMatch = url.match(/admin\/room_booking\/(\d+)/);
    ...
    document.addEventListener('DOMContentLoaded', function() {
        getRoom_bookingIdFromUrl('.btn-edit-link', "{{ route('room.booking.edit', '__ROOM_booking_ID__') }}");
        getRoom_bookingIdFromUrl('.form-end-booking', "{{ route('room.booking.end', '__ROOM_booking_ID__') }}");
        getRoom_bookingIdFromUrl('.btn-cancel-link', "{{ route('room.booking.cancel', '__ROOM_booking_ID__') }}");
    });
</script>
...
<div class="max-w-7xl mx-auto py-10 sm:px-6 lg:px-8">
    ...
    <div class="mb-6">
        <a href="{{ route('room.booking.index') }}" class="text-sm font-medium hover:text-purple-800 flex items-center">
