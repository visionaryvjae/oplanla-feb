...
<div class="max-w-7xl mx-auto py-10 sm:px-6 lg:px-8">
    <div class="bg-white rounded-xl shadow-lg overflow-hidden mb-6">
        <form action="{{ route('room.booking.index') }}" method="GET" class="px-6 py-4 bg-gray-50 border-b border-gray-200">
            <div class="grid grid-cols-1 md:grid-cols-4 gap-4">
                ...
                <div class="flex items-end space-x-2">
                    ...
                    <a href="{{ route('room.booking.index') }}" class="inline-flex items-center px-4 py-2 bg-gray-200 border border-transparent rounded-md font-semibold text-xs text-gray-700 uppercase tracking-widest hover:bg-gray-300 transition ease-in-out duration-150">
                        Reset
                    </a>
                </div>
            </div>
                        ...
                        <table class="min-w-full divide-y divide-gray-200 hidden md:table">
                            ...
                            <tbody class="bg-white divide-y divide-gray-200">
                                @forelse ($bookings as $booking)
                                    @php
                                        $roomsCount = explode(',', $booking->room_types);
                                    ...
                                    <tr class="hover:bg-gray-50 cursor-pointer" onclick="window.location.href = '{{ route('room.booking.show', $booking->id) }}';">
                                        <td class="px-6 py-4 whitespace-nowrap" rowspan="{{ count($roomsCount) }}">
                                            <div class="text-sm font-medium text-gray-900">#{{ $booking->id }}</div>
                                        </td>
                                        <td class="px-6 py-4 whitespace-nowrap" rowspan="{{ count($roomsCount) }}">
                                            <div class="text-sm font-medium text-gray-900">{{ $booking->user->name }}</div>
                                            <div class="text-sm text-gray-500">{{ $booking->bookingRooms ? $booking->bookingRooms->room->provider->provider_name : 'unknown provider' }}</div>
                                        </td>
                                        <td class="px-6 py-4 whitespace-nowrap" rowspan="{{ count($roomsCount) }}">
                                        ...
                                        @endphp
                                        <td class="px-6 py-4 whitespace-nowrap text-sm text-gray-500">{{ $booking->bookingRooms ? $booking->bookingRooms->room->room_type : 'unknown room type'}}</td>
                                    </tr>
                        ...
                        <div class="grid grid-cols-1 gap-4 md:hidden">
                            @forelse ($bookings as $booking)
                                @php
                                    ...
                                    $datestrOut = new DateTime($booking->check_out_time);
                                @endphp
                                <div class="bg-white p-4 rounded-lg shadow space-y-3 cursor-pointer" onclick="window.location.href = '{{ route('room.booking.show', $booking->id) }}';">
                                    <div class="flex justify-between items-start">
                                        <div>
                                            <p class="text-sm font-semibold text-gray-800">{{ $booking->user_name }}</p>
                                            <p class="text-xs text-gray-500">{{ $booking->provider_name }} (ID: #{{$booking->id}})</p>
                                        </div>
                                        <span class="px-2 inline-flex text-xs leading-5 font-semibold rounded-full @if($booking->status == 'confirmed' || $booking->status == 'pending check in') bg-green-100 text-green-800 @elseif($booking->status == 'booking ended') bg-gray-100 text-gray-800 @elseif($booking->status == 'booking canceled') bg-red-100 text-red-800 @else bg-yellow-100 text-yellow-800 @endif">
                                            {{ ucfirst($booking->status) }}
                                        </span>
                                    </div>
                                    <div class="grid grid-cols-2 gap-4 border-t pt-3">
                                         <div>
                                            ...
                                            <p class="text-sm font-medium text-gray-800">{{ $datestr->format('M d, Y') }}</p>
                                            <p class="text-xs text-gray-600">{{ $datestr->format('g:ia') }}</p>
                                        </div>
                                         <div>
                                            <p class="text-xs text-gray-500">Check-out</p>
                                            <p class="text-sm font-medium text-gray-800">{{ $datestrOut->format('M d, Y') }}</p>
                                            <p class="text-xs text-gray-600">{{ $datestrOut->format('g:ia') }}</p>
                                        </div>
                                    </div>
                                    <div class="border-t pt-3">
                                        <div class="flex justify-between items-center">
                                            <div>
                                                <p class="text-xs text-gray-500">Room Types</p>
                                                <p class="text-sm text-gray-800">{{ $booking->room_types }}</p>
                                            </div>
                                            <p class="text-lg font-bold text-gray-900">R {{ number_format($booking->booking_price, 2) }}</p>
                                        </div>
                                    </div>
                                </div>
                            @empty
