        ...
        @if(count($rooms) > 0)
            <form action="{{ route('room.booking.store') }}" method="POST" class="form">
                    ...
                    <tbody class="text-gray-700">
                        ...
                        @foreach($rooms as $index => $room)
                            <tr>
                                ...
                                <td class="text-left py-3 px-4">ZAR {{ $room->price * env('MARK_UP_PERCENT')}} p/night</td>
                            ...
                            @endphp
                        @endforeach
                        <input type="hidden" name="booking_price" id="booking_price" value="{{ $totalPrice * env('MARK_UP_PERCENT') }}">
                        <tr style="border-top: solid rgb(75 85 99) 2px;">
                            <td class="text-left py-3 px-4">Total Price</td>
                            <td class="text-left py-3 px-4"></td>
                            <td class="text-left py-3 px-4">ZAR {{ $totalPrice * config('app.markup') }}</td>
                        </tr>
                    </tbody>
                </table>
                ...
                <div style="display:flex; margin-top: 1.5rem; align-items: center;">
                    ...
                    <a href="{{ route('room.booking') }}"  class="cancel-btn" style="margin-left: 1rem;">
                        cancel
                    </a>
                </div>
            </form>
